...
# 1. Load Summary Data for Dashboard
data_summary = [
...
# 3. App Title and Navigation
st.title("🩺 Sistem Prediksi Diagnosis Diabetes Mellitus")
menu = st.sidebar.selectbox("Pilih Menu", ["Dashboard Performa Model", "Prediksi Diagnosis"])

if menu == "Dashboard Performa Model":
    st.header("📊 Ringkasan Performa Model (Skenario 1-5)")
    st.write("Tabel berikut menunjukkan perbandingan performa antar skenario pengujian:")
    ...
    st.write("Masukkan data laboratorium dan fisik pasien untuk mendapatkan prediksi diagnosis.")
    ...
    if submit_button:
        ...
        # Features: Umur, JK, IMT, Lingkar Perut, Sistolik, Diastolik, Hba1c, GDPuasa, GD2PP
        input_data = pd.DataFrame([[
            umur, jk_val, imt, lingkar_perut, sistolik, diastolik, hba1c, gd_puasa, gd_2pp
        ...
        if prediction == 1:
            st.error("⚠️ Pasien Terdiagnosa: **DIABETES MELLITUS (DM)**")
        else:
